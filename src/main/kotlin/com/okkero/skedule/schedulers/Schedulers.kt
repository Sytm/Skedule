...
object Schedulers {
    ...
    fun global(plugin: Plugin) = if (isFolia) {
        FoliaGlobalScheduler(plugin)
    } else {
        BukkitScheduler(plugin)
    }

    fun region(plugin: Plugin, location: Location) = if (isFolia) {
        FoliaRegionScheduler(plugin, location)
    } else {
        BukkitScheduler(plugin)
    }

    fun entity(plugin: Plugin, entity: Entity) = if (isFolia) {
...
internal class BukkitScheduler(private val plugin: Plugin) : AbstractScheduler {
    ...
    override fun schedule(block: Runnable, retired: Runnable) = BukkitScheduledTask(scheduler.runTask(plugin, block))
...
internal abstract class FoliaSchedulerBase(protected val plugin: Plugin) : AbstractScheduler {
    ...
    override fun scheduleLaterAsync(block: Runnable, delay: Long, retired: Runnable): AbstractScheduledTask =
        FoliaScheduledTask(scheduler.runDelayed(plugin, ConsumerRunner(block), delay * 50, TimeUnit.MILLISECONDS))

    override fun scheduleTimerAsync(block: Runnable, interval: Long, retired: Runnable): AbstractScheduledTask =
        FoliaScheduledTask(
            scheduler.runAtFixedRate(
                plugin,
                ConsumerRunner(block),
                0L,
                interval * 50,
...
internal class FoliaGlobalScheduler(plugin: Plugin) : FoliaSchedulerBase(plugin) {
    ...
    override fun scheduleLater(block: Runnable, delay: Long, retired: Runnable): AbstractScheduledTask =
...
internal class FoliaEntityScheduler(
    ...
    override fun scheduleLater(block: Runnable, delay: Long, retired: Runnable): AbstractScheduledTask? =
        scheduler.runDelayed(plugin, ConsumerRunner(block), retired, delay)?.let { FoliaScheduledTask(it) }
